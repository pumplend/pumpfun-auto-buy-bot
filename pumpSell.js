...
async function loop() {
    ...
    for(i of positions)
    {
        ...
        if(poolSize>miniPoolSize)
        {
            ...
            await sellToken(i.address,i.balance*1e6)
        }else{
            console.log(poolSize,i.address)
        }
...
async function sellToken(tk,amount)
{
  ...
  const instruction = await lend.pump_sell(new PublicKey(tk),kp.publicKey,amount,0);
